...
4) Calculate the mean and sd of the data and then superimpose a normal curve using dnorm(x, mean, sd). Annotate this code as you go using #s so that you can reference it later. Refer back to previous R activities from Unit3 for help if needed.
...
An additional way we can examine if data are normal or not is to make a qqplot. Recall our qqplot lecture notes from last class.
...
#the code below is a for loops. It will repeat a section of code a specified number of times. In this case, we are going to loop through this code 10,000 times. Each loop through the code is designated as i, with i=1 for the first loop through i=10000 for the 10000th loop.
